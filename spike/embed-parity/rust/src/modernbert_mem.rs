...
//! GeGLU MLP, local/global layer schedule and normalization are unchanged; the
//! forward pass is reorganized so a full-length (8192-token) sequence fits in
//! memory:
//!
//!   1. Attention is computed one head at a time. The stock path builds a dense
//!      `(batch, heads, seq, seq)` f32 tensor — 3.2 GB at seq=8192 with 12
//!      heads — and then adds a mask and runs the *unfused* `ops::softmax`,
//!      which allocates three more tensors of that size. Measured peak RSS on
//!      this corpus was 16.3 GB and the process was OOM-killed on a 19 GB box.
//!      Per head the largest tensor is `(seq, seq)` = 268 MB.
...
//!
//! Splitting the batched attention matmul into per-head matmuls is exact (heads
//! are independent), and `softmax_last_dim` performs the same
//! subtract-max/exp/normalize reduction in the same order as `softmax`.
//! `--impl stock` runs candle's original module so the two can be diffed on
//! inputs small enough for the stock path to survive.
...
impl Attention {
    ...
    fn forward(&self, xs: &Tensor, additive_mask: Option<&Tensor>) -> Result<Tensor> {
        ...
        debug_assert_eq!(b, 1, "this spike embeds one sequence at a time");
...
impl Mlp {
    fn load(vb: VarBuilder, config: &Config) -> Result<Self> {
        Ok(Self {
            wi: linear_no_bias(config.hidden_size, config.intermediate_size * 2, vb.pp("Wi"))?,
...
impl Layer {
    fn load(vb: VarBuilder, config: &Config, rotary: Arc<RotaryEmbedding>, local: bool) -> Result<Self> {
        Ok(Self {
...
impl ModernBertMemLite {
    /// `vb` must address the checkpoint's own tensor names — granite ships a
    /// bare `ModernBertModel` (`embeddings.*`, `layers.N.*`, `final_norm.*`),
    /// whereas candle's module prefixes everything with `model.`.
    pub fn load(vb: VarBuilder, config: &Config) -> Result<Self> {
        ...
        let final_norm =
            layer_norm_no_bias(config.hidden_size, config.layer_norm_eps, vb.pp("final_norm"))?;
        ...
        Ok(Self {
            ...
            // The additive mask must match the activation dtype; candle's stock
            // module hardcodes F32 here and therefore cannot run f16 weights.
            dtype: vb.dtype(),
        })
    }
    ...
    /// this spike embeds a single sequence per call), so no padding mask is
    /// taken — the stock path's all-ones mask contributes `-0.0` to every
    /// logit, which changes nothing.
